...
###Hashgraph（有点怪异）
具体可以参考这篇论文 [hashgraph](http://www.swirlds.com/downloads/SWIRLDS-TR-2016-02.pdf)
主要流程可以归纳为
1. 每个节点产生event，并且通过gossip协议发送给其他节点(gossip是从所有节点中选一部分来进行通讯)，每次发送会把自己收到的event也一并发出去
2.  因为每个节点会把自己收到的event也一并发出去,因此其他节点可以通过该event传递操作
3. 每一轮会主动产生一个event，根据接受到的操作判断上一轮的event是否是famous event（被大多数主动产生的event收录）其中famous event收录的操作进行排序来规定操作顺序（event可以包含多个操作，一般使用操作队列中中间的时间戳作为event时间戳）

###DAG(Tagnle, IOTA用的，感觉是一种特殊的hashgraph)
１．　每一笔交易需要验证前两笔未被验证的交易（推荐使用从创始区块到未被确认交易的带权随机游走方案，可能是积累价值越大，或者交易越多的）
２．　ａ)两分钟设置一个里程碑去确认交易（中心化的，确认机构是ＩＯＴＡ基金会）b)设置一个确认信心值，该值的计算为随机选取１００个ｔｉｐｓ（未被确认的交易），其中能索引到目标交易的数量除以１００就是信心值
...
5. 其实，节点ID与文件散列直接对应，它所表示的那个节点存储着哪儿能够获取文件和资源的相关信息
